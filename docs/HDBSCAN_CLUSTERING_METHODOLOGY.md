...
- **현재 버전 (v2, 2025-01)**: [10, 11, 12, 13, 16, 17, 19, 21] - 8개 제품
...
| 지표 | 값 | 해석 |
|------|-----|------|
| **Silhouette Score** | **0.6014** | 우수한 클러스터링 품질. 클러스터 내 응집도가 높고 클러스터 간 분리도가 명확함 |
| **Davies-Bouldin Index** | **0.6872** | 낮은 값으로 우수한 성능. 클러스터 간 분리도가 높음 |
| **Calinski-Harabasz Index** | **6385.79** | 높은 값으로 우수한 성능. 클러스터 간 분산이 클러스터 내 분산보다 훨씬 큼 |
| **클러스터 수** | **19개** | 자동 결정. 데이터 특성에 맞는 적절한 클러스터 수 |
| **노이즈 포인트** | **0.3%** | 매우 낮은 노이즈 비율. 대부분의 샘플이 의미있는 클러스터에 속함 |

#### 6.2.2 K-Means 대비 성능 비교

| 지표 | K-Means | HDBSCAN | 개선율 |
|------|---------|---------|--------|
| **Silhouette Score** | 0.3061 | **0.6014** | **+96.5%** |
| **Davies-Bouldin Index** | 1.2345 | **0.6872** | **-44.3%** |
| **Calinski-Harabasz Index** | 3421.5 | **6385.79** | **+86.6%** |
| **클러스터 수** | 19 (고정) | 19 (자동) | - |
| **노이즈 포인트** | 0% | 0.3% | - |
...
- **프리미엄 제품 재정의 (2025-01)**: 실제 제품명과 가격대 기반으로 프리미엄 제품 번호 업데이트
...
- **Davies-Bouldin Index: 0.6872** (-44.3% 개선)
- **Calinski-Harabasz Index: 6385.79** (+86.6% 개선)
- **클러스터 수: 19개** (자동 결정)
- **노이즈 포인트: 0.3%** (매우 낮음)
...
| **v3** | 6개 핵심 피쳐, 이진 변수 제거 | **0.6014** | **19** | **0.3%** |
...
**권장 사항:**
1. **현재 구조 유지 (권장)**: HDBSCAN은 밀도 기반 클러스터링 알고리즘이므로 다중공선성의 영향이 회귀분석보다 적습니다. 현재 성능(Silhouette Score 0.6014)이 우수하므로 구조를 유지하는 것이 좋습니다.
2. **`age_scaled` 제거 시 성능 저하 확인**: 실험 결과, `age_scaled`를 제거하면 성능이 크게 저하됩니다 (Silhouette Score: 0.6014 → 0.2491, -58.58% 감소). 이는 `age_scaled`가 세그먼트 원-핫 인코딩 변수와는 다른 정보(연령의 세밀한 차이)를 제공한다는 것을 의미합니다.
...
|------|----------------------|-----------------|------|
| **Silhouette Score** | **0.6014** | **0.2491** | **-58.58%** ⚠️ |
| **Davies-Bouldin Index** | **0.6872** | **1.2230** | **+77.9%** ⚠️ |
| **Calinski-Harabasz Index** | **6,385.79** | **990.06** | **-84.5%** ⚠️ |
| **클러스터 수** | **19개** | **60개** | **+215.8%** |
| **노이즈 포인트** | **0.3%** | **25.47%** | **+8,390%** ⚠️ |

**주요 발견:**

1. **성능 크게 저하**
   - Silhouette Score가 0.6014에서 0.2491로 58.58% 감소
   - Davies-Bouldin Index가 0.6872에서 1.2230으로 77.9% 증가 (악화)
   - Calinski-Harabasz Index가 6,385.79에서 990.06으로 84.5% 감소

2. **과도한 세분화**
   - 클러스터 수가 19개에서 60개로 증가 (215.8% 증가)
   - 노이즈 포인트가 0.3%에서 25.47%로 급증 (8,390% 증가)
...
#### 9.2.2 실제 DB 변수 목록 (2025-01 업데이트)
...
1. **우수한 클러스터링 품질**: Silhouette Score 0.6014로 K-Means 대비 +96.5% 향상
2. **의미있는 클러스터**: 생애주기, 소득, 프리미엄 선호도 등 해석 가능한 특성으로 구분
3. **낮은 노이즈 비율**: 0.3%로 대부분의 패널이 의미있는 클러스터에 속함
...
**주요 업데이트 내역:**
- 2025-01-XX: 클러스터 비교 분석 방법론 섹션 추가 (9장)
  - 비교 변수 분류 및 처리 방법
  - 차트별 기본 변수 선정
  - 백엔드 변환 로직 (이진형 변수 자동 감지)
  - 프론트엔드 변수 정의 업데이트
  - 개선 사항 요약
- 2025-01-XX: 프리미엄 제품 정의 업데이트 (3.2.3절)
  - 기존 프리미엄 제품 번호: [3, 9, 18, 20, 22, 25] (6개)
  - 새로운 프리미엄 제품 번호: [10, 11, 12, 13, 16, 17, 19, 21] (8개)
  - 제품명 기반 재정의: 로봇청소기, 무선청소기, 커피머신, 안마의자, 의류관리기, 건조기, 식기세척기, 가정용식물재배기
